/**
 * Swedish Calendar Module
 * Provides Swedish Name Days (Svenska Akademiens namnlängd)
 * and calculates all Swedish Public Holidays (Röda dagar) & traditional eves (Aftnar).
 */

// Full year Swedish Name Days (MM-DD format)
export const SWEDISH_NAME_DAYS = {
  // January
  "01-01": "Nyårsdagen",
  "01-02": "Svea",
  ...
  "05-24": "Ivan, Vanja",
  "05-25": "Urban",
  "05-26": "Vilhelmina, Helmy",
  "05-27": "Beda, Blenda",
  "05-28": "Ingeborg, Gittan",
...
/**
 * Compute all Swedish public holidays & eves for a specific year
 */
export function getSwedishHolidaysForYear(year) {
  const holidays = {};

  const addHoliday = (dateStr, name, isRedDay = true, isEve = false) => {
    holidays[dateStr] = {
      name,
      isRedDay,
      isEve
  ...
  addHoliday(`${year}-01-01`, "Nyårsdagen", true);
  addHoliday(`${year}-01-06`, "Trettondedag jul", true);
  addHoliday(`${year}-05-01`, "Första maj", true);
  addHoliday(`${year}-06-06`, "Sveriges nationaldag", true);
  addHoliday(`${year}-12-25`, "Juldagen", true);
  addHoliday(`${year}-12-26`, "Annandag jul", true);

  // Notable traditional Eves
  addHoliday(`${year}-04-30`, "Valborgsmässoafton", false, true);
  addHoliday(`${year}-12-24`, "Julafton", true, true);
  addHoliday(`${year}-12-31`, "Nyårsafton", true, true);

  // Moveable Easter-based holidays
  const easterSunday = getEasterSunday(year);
  addHoliday(formatDateKey(addDays(easterSunday, -2)), "Långfredagen", true);
  addHoliday(formatDateKey(addDays(easterSunday, -1)), "Påskafton", false, true);
  addHoliday(formatDateKey(easterSunday), "Påskdagen", true);
  addHoliday(formatDateKey(addDays(easterSunday, 1)), "Annandag påsk", true);
  addHoliday(formatDateKey(addDays(easterSunday, 39)), "Kristi himmelsfärdsdag", true);
  addHoliday(formatDateKey(addDays(easterSunday, 49)), "Pingstdagen", true);

  // Midsommar: Midsommardagen is Saturday between June 20-26
  // Midsommarafton is Friday between June 19-25
  for (let day = 19; day <= 25; day++) {
    const d = new Date(Date.UTC(year, 5, day));
    if (d.getUTCDay() === 5) { // Friday
      addHoliday(formatDateKey(d), "Midsommarafton", true, true);
      addHoliday(formatDateKey(addDays(d, 1)), "Midsommardagen", true);
      break;
    }
  }

  // Alla helgons dag: Saturday between October 31 and November 6
  // Allhelgonaafton is the Friday before
  for (let day = 31; day <= 37; day++) {
    // Note: Day 31 is Oct 31, Day 32 is Nov 1, etc.
    const month = day === 31 ? 9 : 10;
    const actualDay = day === 31 ? 31 : day - 31;
    const d = new Date(Date.UTC(year, month, actualDay));
    if (d.getUTCDay() === 6) { // Saturday
      addHoliday(formatDateKey(d), "Alla helgons dag", true);
      addHoliday(formatDateKey(addDays(d, -1)), "Allhelgonaafton", false, true);
      break;
    }
  }
...
// Memory cache of calculated years
const holidayCache = {};

/**
 * Returns Swedish holiday and name day details for a specific date (Date object or YYYY-MM-DD string)
 */
export function getSwedishDayInfo(inputDate) {
  ...
  const nameDay = SWEDISH_NAME_DAYS[mmDdKey] || null;

  if (!holidayCache[year]) {
    holidayCache[year] = getSwedishHolidaysForYear(year);
  }

  const holidayInfo = holidayCache[year][dateKey] || null;

  return {
    dateKey,
    nameDay,
    holiday: holidayInfo ? holidayInfo.name : null,
    isRedDay: holidayInfo ? holidayInfo.isRedDay : false,
    isEve: holidayInfo ? holidayInfo.isEve : false
  };
}
